...
La variable respuesta que analizaremos se denomina **TotCopies** y representa a las copias totales del gen expresado en la gónada de los peces. Las variables predictoras son **DPH** que representa a los días por eclosión (Days post hatching) y **VGLL3** que representa el genotipo de los peces el cual puede ser *EE* asociado a maduración temprana (Early) *LL* asociado a maduración tardía (Late). 
...
Importe el set de datos **vgll3_ancova.xlsx** y realice un análisis exploratorio de datos como se indica a continuación.

**a).** Transforme variable **VLLL3** a factor y realice exploratorio con función **summary()**.
...
**b).** realice histograma de la variable respuesta **TotCopies**.
...
**c).** Realice Boxplot de la variable respuesta **TotCopies** en función del genotipo **vgll3**.
...
Evalue multicolinealidad, normalidad y homogeneidad de varianzas. Use la función **vif()** y los métodos gráficos con la función **qqPlot()** y **plot()** respectivamente.

**a).** Evalue multicolinealidad.
...
```{r}
# Normalidad
qqPlot(fitFull) # library(car)

```

**c).** Evalue homogeneidad de varianzas.
```{r}
...
**b).** Compare los modelos completo usando residuales. Use la función **anova()**.
...
**c).** Compare los modelos completo usando el criterio AIC y BIC.

¿Cuál es el mejor modelo según el criterio AIC?. Use la función **AIC()**.
...
¿Cuál es el mejor modelo según el criterio BIC?. Use la función **BIC()**.

```{r}
# Comparación usando AIC
bic <- BIC(fitFull, fitSimple)

pander::pander(bic, caption = "Comparación modelos usando AIC.")
```